...
export default function Achievements({ achievements, dbError }) {
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false)

  return (
    <>
      <Head>
        <title>Achievements | Ganesh Karadkar</title>
        <meta
          name="description"
          content="Professional achievements, awards, and recognitions earned by Ganesh Karadkar"
        />
      </Head>
      <motion.main
        initial="hidden"
        ...
        <motion.nav variants={itemVariants} className="fixed top-0 z-50 w-full border-b border-slate-800 bg-slate-950/95 backdrop-blur-xl">
          ...
          {mobileMenuOpen && (
            <div id="mobile-navigation" className="grid grid-cols-2 gap-1 border-t border-slate-800 bg-slate-950 px-5 py-3 text-sm text-slate-300 md:hidden">
              ...
              <Link href="/certifications" className="rounded-lg px-3 py-2 hover:bg-slate-900 hover:text-white" onClick={() => setMobileMenuOpen(false)}>Certifications</Link>
        ...
        <div className="pt-24 pb-20 px-6">
          <div className="max-w-6xl mx-auto">
            <motion.div variants={itemVariants} className="mb-12">
              ...
              <p className="text-slate-400 text-lg">Awards, recognitions, and milestones that mark my journey</p>
            </motion.div>

            {achievements.length === 0 ? (
              <motion.div variants={itemVariants} className="rounded-[32px] border border-slate-800 bg-slate-900/80 p-12 text-center">
                <p className="text-slate-400 text-lg mb-4">No achievements added yet.</p>
                <p className="text-slate-500 text-sm">Check back soon for updates on accomplishments and awards!</p>
              </motion.div>
            ) : (
...
export async function getServerSideProps() {
  ...
  try {
    await connect()
    achievements = await Achievement.find({}).sort({ date: -1, createdAt: -1 }).lean()
  } catch (error) {
